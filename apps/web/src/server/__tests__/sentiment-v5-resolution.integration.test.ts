...
const payload = (promptRunId: string) => ({ promptRunId, classifierVersion: SENTIMENT_CLASSIFIER_VERSION, taxonomyVersion: SENTIMENT_TAXONOMY_VERSION });
const count = async (sqlText: string, params: unknown[] = []) => (await client.query<{ n: number }>(`SELECT count(*)::int AS n FROM ${sqlText}`, params)).rows[0].n;
const analysisOf = async (runId: string) =>
	(
		await client.query<{ id: string; status: string; attempts: number; error_code: string | null; verifier_version: string | null; verified_at: string | null }>(
...
const caseOf = async (runId: string) =>
	(
		await client.query<{ status: string; automated_provider_calls: number; total_actual_cost_usd: string; unresolved_targets: unknown[]; provisional_result: unknown; next_attempt_at: string | null }>(
...
const attemptsOf = async (runId: string) =>
	(
		await client.query<{ ordinal: number; phase: string; outcome: string; generation_id: string | null; actual_cost_usd: string | null }>(
...
const rowsOf = async (runId: string) => ({
	...
	aspects: await count("sentiment_aspect_observations a JOIN sentiment_observations o ON o.id = a.observation_id WHERE o.prompt_run_id = $1", [runId]),
	filtered: await count("sentiment_filtered_claims c JOIN sentiment_analyses s ON s.id = c.analysis_id WHERE s.prompt_run_id = $1", [runId]),
});

const cite = (anchorId: string, polarity: "positive" | "negative" | "neutral") => ({ anchorId, polarity });
const positive = (score: number, ...ids: string[]) => ({ category: "positive", score, confidence: 0.9, evidence: ids.map((i) => cite(i, "positive")) });
const negative = (score: number, ...ids: string[]) => ({ category: "negative", score, confidence: 0.8, evidence: ids.map((i) => cite(i, "negative")) });
...
type Step = { phase: "classify" | "repair" | "verify"; answer?: unknown; error?: Error; costUsd?: number; hold?: Promise<void> };
...
function scripted(steps: Step[]): { provider: Provider; script: Script } {
	...
	const provider = {
		...
		access: "api",
		isConfigured: () => true,
		async runStructuredResearch<T>({ prompt, schema, webSearch }: { prompt: string; schema: { parse: (v: unknown) => T }; webSearch?: boolean }) {
			const phase = prompt.startsWith("You are an independent verifier") ? "verify" : prompt.startsWith("You are repairing") ? "repair" : "classify";
			...
			return {
				...
				usage: { inputTokens: 5000, outputTokens: 800, reasoningTokens: 200, costUsd: step.costUsd ?? 0.02, webSearchRequests: phase === "classify" ? 1 : 0, webSearchRequestsConflict: false },
			};
		},
...
async function seedOld(runId: string, version: string, status: "completed" | "failed" | "pending", aspects: string[]) {
	await client.query(`INSERT INTO sentiment_detections (prompt_run_id, brand_id, detector_version, status, mention_count) VALUES ($1, $2, $3, 'mentions', 1) ON CONFLICT DO NOTHING`, [runId, BRAND, SENTIMENT_DETECTOR_VERSION]);
	...
	const observation = await client.query<{ id: string }>(
		...
		[analysis.rows[0].id, mention.rows[0].id, runId, BRAND, JSON.stringify([{ quote: "Sent V5 ERC", start: 0, end: 11, polarity: "negative" }])],
	);
	for (const key of aspects) {
		await client.query(
			...
			[observation.rows[0].id, SENTIMENT_TAXONOMY_VERSION, key, JSON.stringify([{ quote: "Sent V5 ERC", start: 0, end: 11, polarity: "negative" }])],
		);
	}
...
beforeAll(async () => {
	...
	await client.query(`INSERT INTO prompts (id, brand_id, value, enabled, tags, system_tags, created_at, updated_at) VALUES ($1, $2, 'ERC prompt', false, '{}', '{unbranded}', now(), now())`, [PROMPT, BRAND]);
...
async function expectVerifiedCompleted(runId: string, expected: { aspects: number; filtered: number; paidCalls: number }) {
	const analysis = await analysisOf(runId);
	expect(analysis).toMatchObject({ status: "completed", error_code: null, verifier_version: SENTIMENT_VERIFIER_VERSION });
	expect(analysis.verified_at).not.toBeNull();
	expect(await rowsOf(runId)).toEqual({ observations: expect.any(Number), aspects: expected.aspects, filtered: expected.filtered });
...
describe("S1 initial valid → verifier accept → completed_verified", () => {
	it("two paid calls, one attempt-ledger row each, one usage event each, observations persisted only after ACCEPT", async () => {
		const { provider, script } = scripted([{ phase: "classify", answer: { entities: [brandOk([{ key: "coverage", ...positive(80, "s0003") }])] } }, { phase: "verify", answer: ACCEPT }]);
		const outcome = await runSentimentJob(payload(run(1)), { resolveProvider: () => provider, resolutionPolicy: fast });
		expect(outcome).toMatchObject({ status: "classified", verified: true, verifierVersion: SENTIMENT_VERIFIER_VERSION, paidCalls: 2, repairs: 0, verifierRejections: 0, filteredClaimCount: 0 });
		...
		expect((await attemptsOf(run(1))).map((a) => `${a.ordinal}:${a.phase}:${a.outcome}`)).toEqual(["1:classify:accepted", "2:verify:accepted"]);
		expect(await count("usage_events WHERE brand_id = $1 AND prompt_id = $2 AND event_type = 'sentiment_classification'", [BRAND, PROMPT])).toBe(2);
		expect(await runSentimentJob(payload(run(1)), { resolveProvider: () => provider })).toEqual({ status: "already-completed" });
	});
});
...
describe("S2 invalid overall → targeted repair → verifier accept", () => {
	it("the unbound overall becomes a repair target; the repair carries only that key; completed after 3 paid calls", async () => {
		...
		expect(outcome).toMatchObject({ status: "classified", verified: true, paidCalls: 3, repairs: 1, verifierRejections: 0 });
		...
		expect((await attemptsOf(run(2))).map((a) => `${a.phase}:${a.outcome}`)).toEqual(["classify:rejected", "repair:accepted", "verify:accepted"]);
	});
});
...
describe("S3 entity swap → repair → completed", () => {
	it("both swapped entities are repaired in one targeted call; accepted entities are never re-requested", async () => {
		const { provider, script } = scripted([
			{ phase: "classify", answer: { entities: [{ key: "brand", ...positive(75, "s0002"), aspects: [] }, { key: ALPHA, ...positive(75, "s0001"), aspects: [] }] } },
			{ phase: "repair", answer: { entities: [{ key: "brand", ...positive(75, "s0001"), aspects: [] }, { key: ALPHA, ...positive(75, "s0002"), aspects: [] }] } },
			{ phase: "verify", answer: ACCEPT },
		]);
...
describe("S4 structurally valid semantic misclassification → verifier rejects → repair → completed", () => {
	it("the verifier's issue routes the entity to repair; the second verification accepts", async () => {
		const { provider, script } = scripted([
			...
			{ phase: "verify", answer: { verdict: "reject", issues: [{ entityKey: "brand", target: "overall", code: "polarity-mismatch", anchorId: "s0001" }] } },
			{ phase: "repair", answer: { entities: [{ key: "brand", category: "mixed", score: 50, confidence: 0.8, positiveEvidence: [cite("s0003", "positive")], negativeEvidence: [cite("s0001", "negative")], aspects: [] }] } },
			{ phase: "verify", answer: ACCEPT },
		]);
		...
		expect((await client.query("SELECT category FROM sentiment_observations WHERE prompt_run_id = $1", [run(4)])).rows).toEqual([{ category: "mixed" }]);
	});
});
...
describe("S5 / S6 optional aspects are filtered locally and never trigger repair", () => {
	it("S5 one invalid aspect: filtered, no repair, verifier accepts the remaining result", async () => {
		const { provider, script } = scripted([
			{ phase: "classify", answer: { entities: [brandOk([{ key: "coverage", ...positive(80, "s0003") }, { key: "price", ...negative(30, "s0007") }])] } },
			{ phase: "verify", answer: ACCEPT },
		]);
	...
	it("S6 every aspect invalid: completed overall-only, four audit rows, no repair", async () => {
		const { provider } = scripted([
			{ phase: "classify", answer: { entities: [brandOk([{ key: "price", ...negative(30, "s0007") }, { key: "coverage", ...positive(80, "s0006") }, { key: "service", ...positive(70, "s0004") }, { key: "other", ...positive(70, "s0002") }])] } },
			{ phase: "verify", answer: ACCEPT },
		]);
		expect(await runSentimentJob(payload(run(6)), { resolveProvider: () => provider, resolutionPolicy: fast })).toMatchObject({ status: "classified", paidCalls: 2, filteredClaimCount: 4 });
...
describe("S7 first repair invalid, second repair valid", () => {
	it("completed after classify + 2 repairs + verify = 4 paid calls; the ledger shows the rejected repair", async () => {
		...
		expect(await runSentimentJob(payload(run(7)), { resolveProvider: () => provider, resolutionPolicy: fast })).toMatchObject({ status: "classified", paidCalls: 4, repairs: 2 });
		expect((await attemptsOf(run(7))).map((a) => `${a.phase}:${a.outcome}`)).toEqual(["classify:rejected", "repair:rejected", "repair:accepted", "verify:accepted"]);
	});
});
...
describe("S8 transient provider error → bounded backoff → completed", () => {
	it("a 503 is an unpaid attempt, the case waits, the next job run completes; no duplicate charge", async () => {
		...
		await expect(runSentimentJob(payload(run(8)), { resolveProvider: () => provider, resolutionPolicy: fast })).rejects.toMatchObject({ kind: "provider", httpStatus: 503 });
		...
		expect((await attemptsOf(run(8))).map((a) => `${a.phase}:${a.outcome}:${a.generation_id}`)).toEqual(["classify:provider-error:null"]);
		...
		expect(await runSentimentJob(payload(run(8)), { resolveProvider: () => provider, resolutionPolicy: fast })).toMatchObject({ status: "classified", paidCalls: 2 });
...
describe("S9 cost/call ceiling → awaiting_review", () => {
	it("five paid calls exhaust the policy; the case stays open as awaiting_review with the provisional result; nothing persisted", async () => {
		...
		expect(outcome).toMatchObject({ status: "awaiting-review", paidCalls: RESOLUTION_POLICY.maxPaidCalls, unresolved: 1 });
		...
		expect(await runSentimentJob(payload(run(9)), { resolveProvider: () => provider, resolutionPolicy: fast })).toMatchObject({ status: "awaiting-review" });
		expect(script.steps).toHaveLength(1);
		expect((await listUnresolvedCases()).some((u) => u.promptRunId === run(9) && u.status === "awaiting_review")).toBe(true);
	});
	...
	it("the cost ceiling alone also hands over", async () => {
		...
		expect(await runSentimentJob(payload(run(10)), { resolveProvider: () => provider, resolutionPolicy: fast })).toMatchObject({ status: "awaiting-review", paidCalls: 2 });
...
describe("S10 / S11 human adjudication", () => {
	it("S11 an invalid decision (foreign-only overall) is refused by the same deterministic validators; the case stays unresolved", async () => {
		...
		expect(template).toMatchObject({ promptRunId: run(9), status: "awaiting_review", unresolvedTargets: [expect.objectContaining({ entityKey: "brand" })] });
		...
		expect(await rowsOf(run(9))).toEqual({ observations: 0, aspects: 0, filtered: 0 });
		const stale = await applyAdjudication({ analysisId: template.analysisId, inputHash: "0".repeat(64), decidedBy: "operator-test", entities: [brandOk()] });
	...
	it("S10 a valid decision passes the deterministic validators and lands through the same atomic persistence path", async () => {
		...
		const applied = await applyAdjudication({
			...
			inputHash: template.inputHash,
			decidedBy: "operator-test",
			entities: [{ key: "brand", ...positive(70, "s0001", "s0009"), aspects: [{ key: "coverage", ...positive(80, "s0003") }, { key: "price", ...negative(30, "s0007") }] }],
		});
...
describe("S12 / S13 crash safety", () => {
	it("S12 a crash before any request leaves an open case with no attempt row; the next run completes normally", async () => {
		...
		await expect(runSentimentJob(payload(run(12)), { resolveProvider: boom as never, resolutionPolicy: fast })).rejects.toBeDefined();
		expect((await caseOf(run(12))).status).not.toBe("resolved");
		expect(await attemptsOf(run(12))).toEqual([]);
		const { provider } = scripted([{ phase: "classify", answer: { entities: [brandOk()] } }, { phase: "verify", answer: ACCEPT }]);
		expect(await runSentimentJob(payload(run(12)), { resolveProvider: () => provider, resolutionPolicy: fast })).toMatchObject({ status: "classified", paidCalls: 2 });
	});
	...
	it("S13 an attempt left in `sending` (unknown provider outcome) blocks automatic calls: awaiting_reconciliation, zero calls", async () => {
		const { provider, script } = scripted([{ phase: "classify", answer: { entities: [{ key: "brand", ...positive(75, "s0001"), aspects: [] }, { key: ALPHA, ...positive(75, "s0002"), aspects: [] }] } }]);
		// Prepare the analysis + case + a dangling intent row as a crashed worker would leave them.
		const first = await runSentimentJob(payload(run(13)), {
			resolveProvider: () => ({ ...provider, runStructuredResearch: () => new Promise<never>(() => {}) }) as never,
			resolutionPolicy: fast,
			signal: AbortSignal.timeout(300),
		} as never).catch((e: unknown) => e);
		expect(first).toBeDefined();
		const analysis = await analysisOf(run(13));
		await client.query("UPDATE sentiment_provider_attempts SET outcome = 'sending', finished_at = NULL WHERE analysis_id = $1", [analysis.id]);
		const outcome = await runSentimentJob(payload(run(13)), { resolveProvider: () => provider, resolutionPolicy: fast });
		...
		expect((await listUnresolvedCases()).some((u) => u.promptRunId === run(13) && u.status === "awaiting_reconciliation")).toBe(true);
	});
});
...
describe("S14 concurrent workers → one active resolution owner", () => {
	it("two simultaneous jobs: exactly one classifies and verifies; the other returns claimed-elsewhere; two paid calls in total", async () => {
		...
		const { provider, script } = scripted([{ phase: "classify", answer: { entities: [brandOk()] }, hold }, { phase: "verify", answer: ACCEPT }]);
...
describe("S15 persistence failure → no partial rows, case unresolved", () => {
	it("a failing audit write rolls everything back; the analysis is not completed; the case is not resolved; the paid calls stay attributed once", async () => {
		const { provider } = scripted([{ phase: "classify", answer: { entities: [brandOk([{ key: "price", ...negative(30, "s0007") }])] } }, { phase: "verify", answer: ACCEPT }]);
		const persist: typeof persistClassification = (args) =>
			persistClassification({ ...args, classification: { ...args.classification, filteredClaims: args.classification.filteredClaims.map((c) => ({ ...c, code: "not-a-code" as never })) } });
		const before = await count("usage_events WHERE brand_id = $1", [BRAND]);
		await expect(runSentimentJob(payload(run(15)), { resolveProvider: () => provider, resolutionPolicy: fast, persist })).rejects.toMatchObject({ kind: "store", code: "persistence" });
...
describe("S16 / S17 read selection", () => {
	...
	it("S16 failed, pending and unresolved (awaiting_review) v5 never hide a completed v4/v3 analysis", async () => {
		...
		const { provider } = scripted([{ phase: "classify", answer: { entities: [brandUnbound] } }, { phase: "repair", answer: { entities: [brandUnbound] }, costUsd: 0.13 }]);
		expect(await runSentimentJob(payload(run(18)), { resolveProvider: () => provider, resolutionPolicy: fast })).toMatchObject({ status: "awaiting-review" });
		const selected = await selectedSentimentAnalyses();
		expect(selected.filter((s) => s.promptRunId === run(16))).toMatchObject([{ classifierVersion: "sent-classifier-v4" }]);
		expect(selected.filter((s) => s.promptRunId === run(17))).toMatchObject([{ classifierVersion: "sent-classifier-v3" }]);
		expect(selected.filter((s) => s.promptRunId === run(18))).toMatchObject([{ classifierVersion: "sent-classifier-v3" }]);
	});
	...
	it("S17 a completed verified v5 with zero aspects fully replaces the older analysis; none of its aspects leak; an unverified completed v5 row is never read", async () => {
		...
		const { provider } = scripted([{ phase: "classify", answer: { entities: [brandOk()] } }, { phase: "verify", answer: ACCEPT }]);
		expect(await runSentimentJob(payload(run(19)), { resolveProvider: () => provider, resolutionPolicy: fast })).toMatchObject({ status: "classified" });
		expect((await selectedSentimentAnalyses()).filter((s) => s.promptRunId === run(19))).toMatchObject([{ classifierVersion: "sent-classifier-v5" }]);
		const evidence = await loadSentimentEvidence({ ...scope, aspect: "overall", entityKey: "brand", limit: 50 });
		expect([...evidence.highest, ...evidence.lowest].find((i) => i.promptRunId === run(19))).toMatchObject({ score: 70, aspects: [] });
		...
		expect((await selectedSentimentAnalyses()).filter((s) => s.promptRunId === run(20))).toMatchObject([{ classifierVersion: "sent-classifier-v4" }]);
		...
		expect(brandRow?.mentions).toBe(20);
...
describe("S18 every paid response is one attempt-ledger row and one usage event with the actual cost", () => {
	it("ledger rows with a generation id, usage events and case cost totals agree across every scenario", async () => {
		...
		const unpaid = await count("sentiment_provider_attempts t JOIN sentiment_analyses a ON a.id = t.analysis_id WHERE a.brand_id = $1 AND t.generation_id IS NULL", [BRAND]);
		expect(unpaid).toBeGreaterThanOrEqual(2);
		expect(await count("sentiment_provider_attempts WHERE outcome NOT IN ('sending','accepted','rejected','provider-error','aborted')")).toBe(0);
