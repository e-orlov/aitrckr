...
## Pre-existing production incident (not caused by F-08, tracked separately)

Since **2026-09-08 21:17Z** every production `process-prompt` job fails with `OpenRouter API error (403): Key limit exceeded (total limit)` — an account-side limit. The last successful `prompt_runs` rows are from 2026-09-08; 33 jobs keep cycling at the existing 24 h cadence (next due 08:57Z on 09-11). **Production Visibility data is therefore frozen at the last successful 2026-09-08 snapshot** (the carried values shown above are that snapshot). Ingestion is **not** healthy. F-08 changed neither the API key, budget, provider configuration nor the scheduler, triggered no provider request during deployment or acceptance (worker log: 0 provider lines since cutover), and the acceptance oracle recomputed the live values rather than assuming any earlier figure. Resolution of the OpenRouter limit is a separate operational ticket.

## Residual risks

| Risk | Owner | Follow-up |
|---|---|---|
| Frozen ingestion (OpenRouter 403 key limit) — see above | operator | separate incident; after the limit is lifted the scheduler resumes on its own |
