...
- An `issued` permit whose expiry has passed is expired durably — state `expired`, one `permit` audit event — by the next mutating path that observes it: `issuePermit` for the same lifecycle (in the transaction that inserts the replacement) and `consumePermitPhase`. A permit that consumed a phase, or that carries an unresolved `sending` attempt, is never expired, refunded or replaced automatically: that path is reconciliation or revocation only, and an issuer meets it as a typed `PermitConflictError`.
...
| C2 expiry → audit → replacement or reuse → singleton resend after a failed send; typed refusals | `sentiment-resume-verify.integration.test.ts` (IT-SNT-C-008), `sentiment-dispatch-controls.integration.test.ts` (IT-SNT-C-014) |
