...
constants kept in operator evidence (D5).
...
| OPS-SNT-001 | Committed driver `apps/worker/scripts/sentiment-canary.ts` (`inspect` read-only contract form without answer text; `run --contract <file> <run-id>` supervised child, exit 0 only on an accepting verdict; frozen run id only); production contract, run logs and preflight-refusal proofs kept in operator evidence | `pnpm -C apps/worker canary:sentiment …`; evidence `~/.elmo-task-evidence/sent-01/canary-safety/` | PASS (D5) |
