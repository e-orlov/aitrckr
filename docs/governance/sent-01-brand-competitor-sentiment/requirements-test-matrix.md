...
| Sentiment classifier version | `sent-classifier-v3` (v1 retired by the grounded-evidence corrective: the provider cited free-text quotes; v2 cited anchors but typed the entity key as any string and read citation fragments; v3 binds the key to a per-request enum, reads natural-language segments only and hashes the analyzable text) |
