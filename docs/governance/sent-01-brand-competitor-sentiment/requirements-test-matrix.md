...
| Sentiment classifier version | `sent-classifier-v2` (v1 retired by the grounded-evidence corrective: the provider cited free-text quotes; v2 cites anchors) |
| Evidence contract version | `sent-evidence-v1` — the answer is sent as numbered segments `[s0001]…`; evidence is `{ anchorId, polarity }` against a per-request enum of this answer's ids; the code resolves each anchor to the exact raw `{ quote, start, end }`; frozen in the canary contract (`evidenceVersion`) |
