...
- **CI flake, demo citations.spec.ts:36 (runs 33521111801 and 33526358621, attempt 1 each)** — SOLVED after recurrence. retain-on-failure trace from the second occurrence captured the real exception: TypeError: Cannot read properties of undefined (reading …slug…) thrown while building sidebar links — BrandLayout re-renders during a route transition before the next match loader data is committed, passing undefined brand/organization into brandLinkParams → error boundary. Fixes: (1) app guard in the brand layout route — the transition frame now renders the existing pending skeleton instead of crashing (user-facing fix, changeset added); (2) the guard made the transition frame deterministic, exposing the true test-sync defect: citations.spec read textContent(body) immediately after waitForURL — now waits for the page heading first; (3) the same lost-click-before-hydration pattern found in two more menu specs (account-menu.spec, overview.spec) — all four account-menu call sites now use a shared openAccountMenu helper (retry until the menu is actually open). Original wrong classification ("unrecoverable transient") corrected by evidence. The page reached a TERMINAL router error boundary ("Something went wrong", DefaultErrorComponent) right after SPA navigation; the citations route has no loader, so the throw came from the parent _authed route-context load (server-function call) failing transiently on the CI runner. The exact exception is unrecoverable: trace on-first-retry records only the (passing) retry; the dummy SENTRY_DSN swallows the report. Not reproducible locally: dedicated 15-iteration SPA-navigation loop with pageerror/console/requestfailed capture = 0 boundary hits; citations spec ×5 repeats in demo mode = 21/21. Mitigation without masking: playwright config trace retain-on-failure — any future attempt-1 failure now leaves a full trace (console + pageerror + network); the boundary still fails the assertion if it recurs. No blanket timeouts or sleeps added anywhere.
