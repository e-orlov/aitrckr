...
6. Stage E next: test env via CLI `elmo init --dev` flow per `.github/workflows/e2e.yaml` (single source of truth) — Dockerized stack on :1515, seed, run local-mode Playwright + Bruno API baseline; then design dev/test/prod separation (unique compose projects/ports/volumes; NOTE: CLI compose publishes `"5432:5432"` unbound — production must loopback-bind or unpublish postgres, REQ-SEC-001). GATE-B1 after separation (interpretation note in gate record). Baseline E2E expects RUNS_PER_PROMPT=5 default — do not set =1 in test env; =1 goes to production env only (CONF-003).
