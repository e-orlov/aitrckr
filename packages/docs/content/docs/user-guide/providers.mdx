...
On OpenRouter, `:online` in `SCRAPE_TARGETS` is Elmo's web-search flag; the request itself uses OpenRouter's `openrouter:web_search` server tool with `engine: "native"`, so the underlying model's own web search runs where one exists (Anthropic, Google, OpenAI, Perplexity, xAI) and OpenRouter falls back to Exa elsewhere. Every web-enabled OpenRouter request — tracked prompt runs and the onboarding research calls alike — carries a fixed, country-level German searcher location (`country: "DE"`, `timezone: "Europe/Berlin"`, no city or region). This only biases *which search results* the native provider search returns, the way a person searching from Germany would see them; it does not change the IP the request comes from, route traffic through an EU endpoint, or affect where OpenRouter or the model provider process data. Search engines other than the provider's native one ignore the location. Each request requires exactly one web-search step (`tool_choice: "required"`, `max_tool_calls: 1`) instead of letting the model decide to search zero or many times, so cost and behavior stay in line with the one-search-per-request plugin the server tool replaces. `perplexity/sonar` is the cheapest way to track Perplexity: $1/M tokens each way plus $0.005 per request. `perplexity/sonar-pro-search` is closer to the agentic stack perplexity.ai itself serves, at roughly seven times the cost ($3/$15 per M tokens plus $0.018 per request).
