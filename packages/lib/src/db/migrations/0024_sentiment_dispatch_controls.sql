...
CREATE TABLE "sentiment_dispatch_permits" (
	...
	CONSTRAINT "sentiment_dispatch_permits_resume_verify_check" CHECK ("sentiment_dispatch_permits"."purpose" <> 'resume-verify' OR (coalesce(("sentiment_dispatch_permits"."phase_budget"->>'classify')::int, 0) = 0 AND coalesce(("sentiment_dispatch_permits"."phase_budget"->>'repair')::int, 0) = 0 AND "sentiment_dispatch_permits"."contract_sha256" IS NOT NULL))
);
--> statement-breakpoint
