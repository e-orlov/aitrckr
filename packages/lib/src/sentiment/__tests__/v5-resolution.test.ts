...
import { type SentimentCandidate, toClassificationResult } from "../types";
...
const beltra: SentimentCandidate = { key: BELTRA, entityType: "competitor", competitorId: BELTRA, name: "Beltra", aliases: [] };
...
describe("V5-ERC-002 assessCandidate routes instead of rejecting", () => {
	it("valid overall + invalid optional aspect → accepted entity, filtered claim, no unresolved target", () => {
		const a = assessCandidate(
			{ entities: [{ key: "brand", ...positive(75, "s0001"), aspects: [{ key: "price", ...positive(70, "s0002") }] }, { key: BELTRA, ...positive(75, "s0002"), aspects: [] }] },
		...
		expect(a.filteredClaims).toEqual([{ entityKey: "brand", aspectKey: "price", code: "evidence-entity-unbound", anchorIds: ["s0002"] }]);
	});

	it("invalid overall (foreign-only) → that entity becomes an unresolved target; the other entity is accepted", () => {
		const a = assessCandidate(
			{ entities: [{ key: "brand", ...positive(75, "s0002"), aspects: [] }, { key: BELTRA, ...positive(75, "s0002"), aspects: [] }] },
			{ answerBody: answer, candidates: [brand, beltra] },
		);
		expect(a.entities.map((e) => e.key)).toEqual([BELTRA]);
		expect(a.unresolved).toEqual([
			{ entityKey: "brand", reason: "evidence-entity-unbound", aspectKey: null, anchorId: "s0002", source: "deterministic" },
		]);
	});

	it("missing entity → unresolved target `missing-entity`; nothing is invented for it", () => {
		const a = assessCandidate({ entities: [{ key: BELTRA, ...positive(75, "s0002"), aspects: [] }] }, { answerBody: answer, candidates: [brand, beltra] });
		expect(a.unresolved).toEqual([{ entityKey: "brand", reason: "missing-entity", aspectKey: null, anchorId: null, source: "deterministic" }]);
	});
	...
	it("entity swap (996deaca stand-in) → both entities unresolved, none accepted", () => {
		...
		const a = assessCandidate(toClassificationResult(swap.result as never), { answerBody: swap.answer, candidates: swap.candidates });
	...
	it("contract corruption (unknown entity key, duplicate entity) → contractDefect, not a repair target", () => {
		expect(
			assessCandidate({ entities: [{ key: "ARAG", ...positive(75, "s0001"), aspects: [] }] }, { answerBody: answer, candidates: [brand, beltra] }).contractDefect,
		).toEqual({ code: "unknown-entity" });
		expect(
			assessCandidate(
				{ entities: [{ key: "brand", ...positive(75, "s0001"), aspects: [] }, { key: "brand", ...positive(75, "s0001"), aspects: [] }, { key: BELTRA, ...positive(75, "s0002"), aspects: [] }] },
				{ answerBody: answer, candidates: [brand, beltra] },
			).contractDefect,
...
describe("V5-ERC-003 repair and verifier contracts", () => {
	...
	it("the repair schema binds exactly the unresolved keys and the answer's anchors; web search is off by contract", () => {
		...
		expect(schema.safeParse({ entities: [{ key: "brand", ...positive(75, "s0001"), aspects: [] }] }).success).toBe(true);
		expect(schema.safeParse({ entities: [{ key: BELTRA, ...positive(75, "s0002"), aspects: [] }] }).success).toBe(false);
		expect(schema.safeParse({ entities: [{ key: "brand", ...positive(75, "s0042"), aspects: [] }] }).success).toBe(false);
		const prompt = buildRepairPrompt({
			answerBody: answer,
			candidates: [brand, beltra],
			targets: [{ entityKey: "brand", reason: "evidence-entity-unbound", aspectKey: null, anchorId: "s0002", source: "deterministic" }],
		});
	...
	it("the verifier schema is accept-or-bounded-issues with enumerated codes, keys and targets", () => {
		expect([...VERIFIER_ISSUE_CODES].sort()).toEqual(
			["aspect-misrouted", "category-mismatch", "entity-misattribution", "mixed-semantics", "polarity-mismatch", "unsupported-evaluation"].sort(),
		);
		const schema = verifierResultSchemaFor(anchors, ["brand", BELTRA]);
		expect(schema.safeParse({ verdict: "accept", issues: [] }).success).toBe(true);
		expect(schema.safeParse({ verdict: "reject", issues: [{ entityKey: "brand", target: "overall", code: "entity-misattribution", anchorId: "s0002" }] }).success).toBe(true);
		expect(schema.safeParse({ verdict: "reject", issues: [{ entityKey: "ARAG", target: "overall", code: "entity-misattribution", anchorId: null }] }).success).toBe(false);
		expect(schema.safeParse({ verdict: "reject", issues: [{ entityKey: "brand", target: "overall", code: "hallucinated", anchorId: null }] }).success).toBe(false);
		expect(schema.safeParse({ verdict: "accept", issues: [{ entityKey: "brand", target: "overall", code: "polarity-mismatch", anchorId: null }] }).success).toBe(false);
		const prompt = buildVerifierPrompt({
			answerBody: answer,
			candidates: [brand, beltra],
			candidate: { entities: [{ key: "brand", ...positive(75, "s0001"), aspects: [] }, { key: BELTRA, ...positive(75, "s0002"), aspects: [] }] },
		});
	...
	it("verifier issues map to entity repair targets; aspect issues target the entity, never a Neutral placeholder", () => {
		expect(
			...
			{ entityKey: "brand", reason: "verifier:aspect-misrouted", aspectKey: "price", anchorId: null, source: "verifier" },
			{ entityKey: "brand", reason: "verifier:polarity-mismatch", aspectKey: null, anchorId: "s0001", source: "verifier" },
			{ entityKey: BELTRA, reason: "verifier:entity-misattribution", aspectKey: null, anchorId: "s0002", source: "verifier" },
		]);
	});
	...
	it("merging a repair replaces only the targeted entities and keeps accepted ones byte-identical", () => {
		...
		const merged = mergeRepairedEntities(candidate, { entities: [{ key: "brand", ...positive(70, "s0001"), aspects: [] }] }, ["brand"]);
		...
		expect(() => mergeRepairedEntities(candidate, { entities: [{ key: BELTRA, ...positive(70, "s0002"), aspects: [] }] }, ["brand"])).toThrow(/not a repair target/);
	});
});
